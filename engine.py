...
def iphd_evaluation(boxes,target_boxes):
    ...
    return AP, n_frames, n_ave_boxes, n_ave_target_boxes
...
@torch.no_grad()
def evaluate(epoch, model, data_loader, tbwriter, tempdir, device, hot,max_iterations=1000):
    ...
    for images, imfilenames, targets in metric_logger.log_every(data_loader, 100, header):
        if (cc > max_iterations):
            break
        ...
        # im = images[0].cpu()
        # im = converter(im)
        # draw_evaluation(outfilename,im,boxes,target_boxes, hot=hot, preprocess=False)



        cc = cc + 1
    ap, n_frames, n_ave_boxes, n_ave_target_boxes = iphd_evaluation(boxes_dict, target_boxes_dict)
    print("\tAP-25: {:.5f} AP-50: {:.5f} AP-75: {:.5f} Ave n boxes: {} Ave n target boxes: {} in {} frames".format(ap[25],ap[50],ap[75],n_ave_boxes,n_ave_target_boxes,n_frames))

    tbwriter.add_scalar('Validation/AP-25', ap[25], epoch)
    tbwriter.add_scalar('Validation/AP-50', ap[50], epoch)
    tbwriter.add_scalar('Validation/AP-75', ap[75], epoch)
    tbwriter.add_scalar('Validation/nboxes', n_ave_boxes, epoch)
...
@torch.no_grad()
def save_one_epoch(model, data_loader, outputfile, device, hot):
