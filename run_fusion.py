...
#             ret = bbox_iou(box_th[np.newaxis, :], np.asarray(boxes_depth))
...
#             res_scores.append((score_th+scores_depth[box_index])/2)
#             boxes_depth.pop(box_index)
#             scores_depth.pop(box_index)
#         elif(score_th>0.1):
#             res_boxes.append(box_th)
#             res_scores.append((score_th+score_th)/2)
#     for box_d, score_d in zip(boxes_depth, scores_depth):
#         if (score_d > 0.3):
#             res_boxes.append(box_d)
#             res_scores.append((score_d + 0.3) / 2)
#
...
def merging(depth_modelpicklename, thermal_modelpicklename, outputfile, imdir_depth,  imdir_thermal, ground_truth, output_debug, opt, draw_debug=False):
    ...
    with open(depth_modelpicklename.replace('.pkl','_predictions.pkl'), 'rb') as f:
        ...
        with open(thermal_modelpicklename.replace('.pkl','_predictions.pkl'), 'rb') as f:
            ...
            for kvp_depth in depth_dict.items():
                ...
                if(opt=='vanilla'):
                    ...
                    # if (len(boxes_th) > 0):
                    #     boxes_temp = np.array(boxes_th)
                    #     scores_temp = np.array(scores_th)
                    #     inds = nms(boxes_temp, scores_temp)
                    #     boxes_th = [boxes_th[ii] for ii in inds]
                    #     scores_th = [scores_th[ii] for ii in inds]

                    # if (len(boxes_d) > 0):
                    #     boxes_temp = np.array(boxes_d)
                    #     scores_temp = np.array(scores_d)
                    #     inds = nms(boxes_temp, scores_temp)
                    #     boxes_d = [boxes_d[ii] for ii in inds]
                    #     scores_d = [scores_d[ii] for ii in inds]
