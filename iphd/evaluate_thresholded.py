...
if __name__ == "__main__":
    ...
    if(os.path.exists(os.path.join(ref_path, 'ground_truth.pkl'))):
        ...
        with open(os.path.join(output_path, filename.replace('.pkl','-th.txt')), 'w') as f:
            for th in[0, 0.2, 0.5, 0.7]:

                red_predictions = dict()

                for kvp in predictions.items():
                    label = kvp[0]
                    values = kvp[1]

                    new_values = list(filter(lambda v: v[4] >= th, values))
                    red_predictions[label] = new_values
                ...
                for iou in [25, 50, 75]:
                    AP[iou] = eval_ap(red_predictions, ground_truth, iou/100.)


                    res[iou] = round(AP[iou], nb_decimals)
                f.write('{}: {} {} {}\n'.format(th, res[50],res[25],res[75]))
                print('{}: {} {} {}'.format(th, res[50], res[25], res[75]))
